...
const LessonDetailHeader = ({
  ...
  return (
    <div className={`hidden lg:block border-b border-gray-200 px-8 py-5 ${style.light}`}>
      {/* Breadcrumb */}
      <div className="flex items-center gap-2 text-sm text-gray-500 mb-3">
        <Link href="/lessons" className="flex items-center gap-1 hover:text-gray-700">
          <ArrowLeft className="w-4 h-4" />
          <span>Lessons</span>
        </Link>
        <span>/</span>
        <span className="text-gray-700 font-medium truncate max-w-xs">{lesson.name}</span>
      </div>
      ...
      <div className="flex items-center gap-3 mb-4">
        ...
        <div>
          ...
          <div className="flex items-center gap-2 mt-1">
            ...
            <span className={`text-xs font-semibold px-2.5 py-1 rounded-full ${isFinished ? "bg-gray-100 text-gray-600" : "bg-primary-100 text-primary-700"}`}>
      ...
      <div className="flex items-center gap-3 flex-wrap">
        <div className="flex items-center gap-2 bg-white px-3 py-1.5 rounded-full text-sm text-gray-600 border border-gray-200 shadow-sm">
        ...
        <div className="flex items-center gap-2 bg-white px-3 py-1.5 rounded-full text-sm text-gray-600 border border-gray-200 shadow-sm">
          <span className={`w-2 h-2 rounded-full ${attendanceRate >= 80 ? "bg-primary-500" : attendanceRate >= 50 ? "bg-warning-500" : "bg-gray-400"}`} />
