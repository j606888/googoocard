...
const InvitationCard = ({ inviteToken, onDelete }: { inviteToken: InviteToken, onDelete: (id: number) => void }) => {
  return <div className='border border-gray-200 rounded-sm p-3 flex flex-col gap-2'>
    <div className='flex items-center justify-between'>
      ...
      <Trash className='w-4 h-4 text-gray-500' onClick={() => onDelete(inviteToken.id)} />
    </div>
    ...
    <button className='bg-primary-500 text-white px-3 py-2 rounded-sm flex items-center gap-2 justify-center' onClick={() => {
